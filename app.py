# Patch for Python 3.12+ compatibility with snscrape
import sys
if sys.version_info >= (3, 12):
    import importlib.util
    import importlib.machinery
    
    # Monkey patch the old importlib API that snscrape uses
    class LegacyModuleFinder:
        def find_module(self, fullname, path=None):
            try:
                ...
                if loader is None:
                    return None
                return LegacyModuleLoader(loader, spec)
    ...
    # Patch the FileFinder to have find_module
    import importlib._bootstrap_external
    original_find_spec = importlib._bootstrap_external.FileFinder.find_spec
    
    def patched_find_spec(self, fullname, target=None):
        try:
            return original_find_spec(self, fullname, target)
        except Exception:
            pass
        ...
        if result:
            # Create a spec-like object
            class LegacySpec:
                def __init__(self, loader):
                    self.loader = loader
                    self.name = fullname
            return LegacySpec(result)
        return None
    
    importlib._bootstrap_external.FileFinder.find_spec = patched_find_spec
    
    # Also patch the importer interface
    import importlib.machinery
    if not hasattr(importlib.machinery.FileFinder, 'find_module'):
        def find_module(self, fullname, path=None):
            ...
            if spec is None:
                return None
            return LegacyModuleLoader(spec.loader, spec)
